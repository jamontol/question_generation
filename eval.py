...
def main():
    ...
    DF = pd.DataFrame(columns=['sentencia','respuesta', 'pregunta', 'pred'])
    with torch.no_grad():
        for i, batch in enumerate(test_dataloader):
            ...
            sentencia=[vocabs['src_vocab'].itos[num] for num in sentence.data[0].tolist()] 
            pregunta=[vocabs['trg_vocab'].itos[num] for num in question.data[0].tolist()] 
            respuesta=[vocabs['src_vocab'].itos[num] for num in answer.data[0].tolist()] 

            lista_sentencias.append(sentencia)
            lista_respuesta.append(respuesta)
            lista_preguntas.append(pregunta)
            lista_pred.append(pred)
        
        DF['sentencia'] = lista_sentencias
        DF['respuesta'] = lista_respuesta
        DF['pregunta'] = lista_preguntas
        DF['pred'] = lista_pred
